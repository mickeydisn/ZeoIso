...
function InterfaceMap(mainDiv, world) {
...
InterfaceMap.prototype = {
  ...
  createEmptyCanvas: function (width, height, name) {
  ...
  requestFloreChunk: function (cx, cy, callback){
  ...
  requestBuildingChunk: function (cx, cy, callback){
 ...
 appliedZoomLvl: function (n, grain=null) {
        var size = this.world.size;
        var half = size / 2;
  ...
  keyControle: function (keyPressed) {
    var move = false;
    if (keyPressed == '3') {
      if (this.zoom > 0.2) {
        move = true;
  ...
  keyLoopControle: function (keyPressed) {
    var move = false;
    if (keyPressed['d']) {
      move = true;
